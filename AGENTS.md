...
│   ├── reflect.ts               ← scheduled learning engine + prompt templates + reflectCatchUp() backlog-drain runner (D5) + opinion gates, belief journal, counter-evidence pass & falsifier weakening (issue #38) + step-0.5 procedural-suggestion pass integration (issue #39)
│   ├── insight-shared.ts        ← gate/similarity/JSON-parse helpers shared by reflect.ts + suggest.ts (issue #39 pure-move extraction, zero behavior change)
...
├── tests/                        ← TS suites incl. aql-* cross-process (630 tests via npm test; +129 from integrations/pi, +67 from tools/openclaw-import)
│   ├── helpers.ts
│   ├── retain.test.ts
│   ├── retain-gate.test.ts
│   ├── recall.test.ts
